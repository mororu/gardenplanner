...
| `npm run smoke:sicht`       | Fährt Chrome **kopflos** über das DevTools-Protokoll und misst, was gerendert herauskommt: Geometrie bei 375px, berechnete Stile, beide Erscheinungsbilder. Braucht `npm run build` und einen Chrome auf der Maschine (`CHROME_PFAD` überschreibt die Suche). |
| `npm run lint`              | `prettier --check`, `eslint`, `gate`, `gate:selftest`, `db:check`, `db:check:selftest`, `smoke`, `smoke:selftest`, `smoke:http`, `smoke:sicht`. **Braucht einen aktuellen `npm run build`** — `smoke:http` und `smoke:sicht` messen den gebauten Baum.        |
...
`name="neuerName"`, die `value`-Bindung, `aria-invalid`, `aria-describedby`, der
gesperrte Absendeknopf und das `open={fehlerHier}` am `<details>`, das die
abgewiesene Zeile aufgeklappt lässt. Die dritte fasst vier Zusagen, die sonst an je einer
...
reihenfolgeunabhängig geprüft, ausdrücklich **nicht** hinter einem `{#if}` und
mit einem Rumpf, der nur der abgewiesenen Zeile gilt.
...
**Vier Mutationen aus Story 3.0.1 blieben zuerst grün, und alle vier waren
Löcher.** Sie stehen hier, weil das Nachziehen der Prüfung der eigentliche Ertrag
war:
...
- Die `smoke:http`-Behauptung über das versteckte `mitgliedId` suchte zuerst über
  ...
  findet, prüft nichts.
- `open={fehlerHier}` vom `<details>` entfernt lief grün durch `check`, `smoke`
  **und** `smoke:http` — die Textprüfung schnitt erst am `<form>`, das Attribut
  steht davor, und die HTTP-Prüfung sah nur die ausgelieferte Liste, nie eine
  Antwort auf einen abgewiesenen POST. Ausgerechnet die stärkste Zusage dieser
  Story hing damit an nichts: ohne `open` liefert der Server nach einer Abweisung
  ein zugeklapptes Formular, und ohne JavaScript gibt es nichts, was es
  nachträglich aufklappte. Der Schnitt beginnt jetzt am `<details>`, und
  `smoke:http` schickt einen echten POST ab.
- Der Satz zur Zeile auf `{fehlerAmNeuenNamen}` ohne Zeilenbezug lief ebenfalls
  grün: die Live-Region-Behauptung las den **Tag**, nicht den Rumpf. Die Mutation
  hätte denselben Fehlersatz in jede aktive Zeile gestellt — zwanzig assertive
  Regionen, die im selben Augenblick dasselbe ansagen, ohne dass eine von ihnen
  sagt, um wessen Namen es geht. Gedeckt ist der Rumpf jetzt als Text und als
  Zählung am ausgelieferten Dokument.
...
`/monatsplan`, `/aufgabe`, `/dienstplan`) steht ausserdem: kein unersetzter
...
grundsätzlich nicht messen kann: dass `/verwaltung` das Umbenennen ohne
JavaScript wirklich trägt. Zwei Behauptungen, eine je Richtung.

Die erste liest die ausgelieferte Liste: jedes Formular mit
`action="?/umbenennen"` wird einzeln aus dem HTML geschnitten, und in seinem
Rumpf stehen `method="POST"`, das Feld `neuerName`, die versteckte Zeilen-Id mit
einer echten Zahl und der Absendeknopf — je aktiver Zeile eines, zwei bei der
Saat.

Die zweite schickt einen **echten POST** ab, mit einem Namen aus 81 Zeichen, und
misst das Dokument, das zurückkommt: `400`, `text/html`, genau **ein**
aufgeklapptes `<details>` im ganzen Dokument, darin die verworfene Eingabe, das
`aria-invalid="true"` und der Verweis auf die Live-Region dieser Zeile — und der
Satz genau in dieser einen Region und in keiner anderen. Es ist die einzige
POST-Behauptung des Skripts, und der Grund für die Ausnahme ist zugleich ihre
Rechtfertigung: `smoke` ruft die action direkt und sieht nie ein Dokument, und
die Zusage lautet nicht „die action weist ab", sondern „das Dokument kommt
fertig aus dem Server". Ohne sie stand genau diese Zusage ungedeckt da — das
`open` am `<details>` entfernt lief grün durch die ganze Kette.
