...
**Der Preis dafür ist bezahlt und benannt:** dieser Wurf verlässt SvelteKit über den Fatal-Pfad, deshalb trägt die Seite keinen Rahmen und keine `set-cookie`-Kopfzeile. Beides ist gemessen, beides steht so in der Matrix, und beides ist verkraftbar — die Navigationsleiste wäre für jemanden ohne Zugang ohnehin ein Angebot ins Leere, und ein liegengebliebenes Cookie öffnet nichts, weil `is_active` bei jedem Aufruf frisch aus der Datenbank kommt. Was **nicht** verkraftbar wäre: dass diese Eigenschaft nur heute gilt. Darum prüft `scripts/smoke-zugang.ts` sie ausgeführt, nicht per Augenschein.
