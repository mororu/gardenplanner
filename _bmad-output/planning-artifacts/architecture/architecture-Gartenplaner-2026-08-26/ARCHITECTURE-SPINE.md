...
- **Rule:** Die Startseite `/` führt genau drei Blöcke in dieser Reihenfolge: (1) einen Hinweis, falls die betrachtende Person diese Woche Dienst hat, (2) offene Einzelaufgaben ohne Übernehmer, (3) den offenen Aufgaben-Pool. Eine neue Aufgabenart erscheint nur dann in der Anwendung, wenn sie auch hier einsortiert wird. Unterseiten dürfen vertiefen, nie exklusiv informieren. **Die Zahl eines Blocks steht in seiner Überschrift; ein Verweis ohne eigene Aufgabenart darf danebenstehen und informiert nie exklusiv.**

> **Ergänzt am 2026-09-11, und am selben Tag noch einmal gefasst.** Die erste Fassung erlaubte ein **Überblicksband** über den drei Blöcken: drei Kacheln mit den offenen Aufgaben, den freien Einzelaufgaben und den unbesetzten Tränkewochen. Gebaut und wieder aufgelöst — es stellte sich heraus, dass es dieselben Abschnitte ein zweites Mal überschrieb: das Band sagte „4 Aufgaben offen", und die Marke darunter sagte „Offen" über derselben Liste. Zwei Überschriften für eine Sache, gemessene 251px Kopfbereich.
>
> **Jetzt ist die Zahl die Überschrift.** Jeder der zwei Blöcke auf `/` ist ein Aufklapper, dessen Griff seine Zahl trägt (`4 Aufgaben offen · 2 überfällig`), und bei null trägt er statt der Zahl einen Satz. Der Kopfbereich misst so 180px. Der eigentliche Gewinn ist aber ein anderer: **ein zugeklappter Block verbirgt damit seinen Inhalt, nicht mehr seine Lage** — und die Prevents-Klausel dieser Regel hält unabhängig davon, ob jemand die Blöcke offen lässt. Vorher hing sie allein am `open`.
>
> Die unbesetzten Tränkewochen haben auf dieser Seite keinen Block, weil der Plan eine eigene Seite ist. Sie stehen als **Verweis** neben dem Diensthinweis — mit Pfeil statt Dreieck, weil gleiche Form bei verschiedenem Verhalten die Falle wäre. Die Schranke bleibt die alte: wer eine Aufgabenart nur in eine Überschrift oder einen Verweis schreibt und nicht in einen der drei Blöcke, verstösst gegen AD-14.
...
>
> Dieser Abschnitt setzte — wie die *Deferred*-Zeile weiter unten — voraus, *Stufe C* hiesse Playwright und NFR13 fiele dem Sinn nach. **Die Prämisse war falsch.** Node bringt seit 22 ein globales `WebSocket` mit, Chrome spricht das DevTools-Protokoll darüber, und ein Prozessstart ist `spawn`. `scripts/kopfbrowser.ts` ist reines Node — 349 Zeilen, zur Hälfte Begründung —, und `scripts/smoke-sicht.ts` legt darauf 52 Behauptungen am gerenderten Ergebnis ab. NFR13 lautet präzisiert *keine fremde Abhängigkeit für Prüfung* — und ist gehalten.
>
